  ...
  for (let i = 0; i < data.length; i++) {
    if (acceptableNames.includes(data[i].AreaName.replace(" ", ""))) {
      ...
      const model = models.filter(
        (model) => model.collection.name === row.AreaName.replace(" ", "")
      )[0];
      let entry = new model(newRow);
      const update = { $set: newRow };
      model.collection.updateOne({ DateTime: newRow.DateTime }, update, {
        upsert: true,
      });
    }
  }
...
ATLNewDataConsumer.on("ready", function () {
  ...
  .on("data", function (data) {
    ...
    let debug = {
      message:
        "ATL Prepare Service received new data and is now populating the database with it",
      data: myobj,
    };
    send_debug(debug);
  });
