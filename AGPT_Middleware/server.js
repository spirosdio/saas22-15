...
app.use("/:dateFrom&:country&:ProductionType", (req, res) => {
  ...
  let debug = {
    message:
      "AGPT Middleware sent message to kafka requesting: " +
      country +
      " from " +
      dateFrom +
  ...
  p.then(() => {
    debug = {
      message:
        "AGPT Middleware received message from kafka: " +
        country +
        " from " +
        dateFrom +
        " with production type" +
...
app.listen(process.env.AGPT_MIDDLEWARE_PORT, () =>
  console.log("Listening on port " + process.env.AGPT_MIDDLEWARE_PORT)
);
