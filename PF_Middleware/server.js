...
app.use("/:dateFrom&:countryFrom&:countryTo", (req, res) => {
  ...
  let debug = {
    message:
      "PF Middleware sent message to kafka requesting: " +
      countryFrom +
      " to " +
      countryTo +
      " from " +
  ...
  p.then(() => {
    debug = {
      message:
        "PF Middleware received message from kafka: " +
        countryFrom +
        " to " +
        countryTo +
        " from " +
...
app.listen(process.env.PF_MIDDLEWARE_PORT, () =>
  console.log("Listening on port " + process.env.PF_MIDDLEWARE_PORT)
);
