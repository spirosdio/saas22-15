...
function csvToArray(csvFilePath, callback) {
    ...
    const header = ['DateTime', 'ResolutionCode', 'OutAreaCode', 'OutAreaTypeCode', 'OutAreaName', 'OutMapCode', 'InAreaCode', 'InAreaTypeCode', 'InAreaName', 'InMapCode', 'FlowValue', 'UpdateTime'];
...
function queueMessage(){
    ...
    const path = files_location + 'CBF/' + date.replace('-', '_').replace('-', '_').replace('-', '_') + '_PhysicalFlows12.1.G.csv';
    ...
    csvToArray(path, function(data) {
        ...
        stream.write(Buffer.from(JSON.stringify(data)), (err) => {
            if (err) {
                ...
                let debug = {
                    message: 'Time changed to ' + date + ' so CBF_Download_Service downloaded the new data and sent it to Kafka.',
                    data: data,
                }
                send_debug(debug);
            }
